          ...
          <section>
            <div className="container mx-auto grid gap-8 grid-cols-1 md:grid-cols-1 lg:grid-cols-1">
              {solutions.map((solution, index) => (
                <div
                  key={index}
                  className={`bg-white p-6 selection:rounded-lg shadow-md flex flex-col md:flex-row justify-center  gap-5 rounded-xl ${
                ...
                >
                  ...
                  <div className="text-left">
                    <div className=" flex items-center gap-5 justify-center pt-10">
                      <p>
                        {solution.title === "Education" && (
                          <MdSchool className="text-3xl" />
                        )}
                        {solution.title === "Information technology" && (
                          <GiTechnoHeart className="text-3xl" />
                        )}
                        {solution.title === "Service" && (
                          <RiCustomerService2Fill className="text-3xl" />
                        )}
                        {solution.title === "Sales" && (
                          <FaCashRegister className="text-3xl" />
                        )}
                        {solution.title === "Marketing" && (
                          <MdHomeRepairService className="text-3xl" />
                        )}
                        {solution.title === "Healthcare" && (
                          <MdHealthAndSafety className="text-3xl" />
                        )}
                        {solution.title === "Finance" && (
                          <FaCommentsDollar className="text-3xl" />
                        )}
                      </p>
